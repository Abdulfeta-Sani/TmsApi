...
public class AuthController : ControllerBase
{
        ...
        [FromBody] LoginRequest request)
    {
        ...
        {
            return Unauthorized(
                new { detail = "Invalid credentials." });
        }
        ...
        var accessToken = _tokenService.GenerateJwt(user, roles);  // Generate short-lived JWT access token
    ...
    [FromBody] RefreshRequest request)
    {
        ...
        {
            return Unauthorized(
                new { detail = "Invalid refresh token." });
        }
        ...
        {
            return Unauthorized(
                new { detail = "User no longer exists." });
        }

        var roles = await _userManager.GetRolesAsync(user);
        var newAccessToken = _tokenService.GenerateJwt(user, roles); // Generate new access token
