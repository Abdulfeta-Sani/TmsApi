    ...
    LinkGenerator linkGenerator) : ControllerBase
{
    ...
    public async Task<IActionResult> GetCourseById(int id, CancellationToken ct)
    {
        ...
        var updateLink = linkGenerator.GetPathByName(HttpContext, nameof(UpdateCourse), new { id })!;
        var deleteLink = linkGenerator.GetPathByName(HttpContext, nameof(DeleteCourse), new { id })!;
    ...
    {
        var createdId = await mediator.Send(new CreateCourseCommand(request.Code, request.Title, request.MaxCapacity), ct);
    ...
    {
        var updated = await mediator.Send(new UpdateCourseCommand(id, request.Code, request.Title, request.MaxCapacity), ct);

        if (!updated)
        {
            var courseExists = await courseService.GetByIdAsync(id, ct);
            if (courseExists is null) return NotFound();
